    ...
    >
              ...
              return (
                <button
                  ...
                  className={`flex cursor-pointer flex-col items-center justify-center rounded-2xl px-2 py-3 text-center transition-all duration-200 ${
                    isActive
                      ? "bg-indigo-700 text-white shadow-[0_14px_28px_rgba(67,56,202,0.28)]"
      ...
      <div className="mb-32 space-y-6">
        <section className="space-y-4 p-4" id="profile" ref={registerSection("profile")}>
          {companyBrand ? (
            <div className="absolute left-6 top-6 z-50">
              {companyBrand.logoSrc ? (
                <img className="h-12 w-auto object-contain" src={companyBrand.logoSrc} alt={company?.name || "Company logo"} />
              ) : (
                <div className="inline-flex min-h-14 min-w-14 items-center justify-center rounded-2xl bg-indigo-700 px-4 text-sm font-bold tracking-[0.18em] text-white">
          ...
          <div className="relative min-h-[520px] overflow-hidden rounded-xl bg-[linear-gradient(145deg,rgba(99,102,241,0.22)_0%,rgba(238,242,255,0.96)_42%,rgba(255,255,255,0.98)_100%)]">
            ...
            <div className="absolute bottom-5 z-50 rounded-r-xl bg-white/70 p-4 backdrop-blur-xs">
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-black drop-shadow-lg md:text-2xl">{profile?.name || "Nama Profil"}</h1>
                <div>
              ...
              <div className="text-indigo-800">
                ...
                <p className="text-lg font-bold">{profile?.licenseNumber || "Nomor Izin"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <a
              ...
              className="inline-flex w-full items-center justify-center gap-3 rounded-xl bg-indigo-700 px-7 py-2 text-white"
              {...renderLinkTarget(companyVideoUrl)}
            >
              <FontAwesomeIcon icon={faDownload} className="text-2xl" />
              <span className="w-fit text-center text-wrap md:text-lg">Download Company Brochure</span>
            </a>

            <a
              href={vcardHref}
              download={`${vcardName || "contact"}.vcf`}
              className="inline-flex w-full items-center justify-center gap-3 rounded-xl border border-indigo-200 px-5 py-2 text-indigo-800"
            >
              <FontAwesomeIcon icon={faAddressBook} className="text-2xl" />
              <span className="w-fit text-center text-wrap md:text-lg">Save Contact</span>
            </a>
          </div>

          <div className="mt-5">
            <h5 className="text-2xl font-bold">Profile</h5>
            <p className="text-justify md:text-lg">{profile?.description || "Tidak ada deskripsi profil."}</p>
          </div>

          <div className="mt-7 grid grid-cols-4 gap-2">
            {infoItems.map((item) => (
              <div key={item.id} className="aspect-square rounded-3xl border border-indigo-200 bg-indigo-50 p-2 text-center">
        ...
        <section id="legality" ref={registerSection("legality")} className="space-y-4 bg-indigo-50/70 p-4">
          <div className="flex items-center justify-between">
            <h5 className="text-2xl font-bold">Legal Integrity</h5>
            <div className="w-fit rounded-full border border-indigo-300 bg-indigo-100 px-2 py-1 text-indigo-800">
        ...
        <section id="corporate" ref={registerSection("corporate")} className="scroll-mt-28 space-y-4 p-4">
          ...
          <div className="space-y-8">
            <div className="space-y-2">
              <h6 className="text-xl font-bold text-indigo-800">{company?.name || "Profil Perusahaan"}</h6>
              <p className="text-justify md:text-lg">
                {company?.description || "Isi desain dan copy Equityworld nanti kamu bisa bedakan di sini."}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-0.5 overflow-hidden rounded-3xl text-center shadow">
              {corporateStats.map((item) => (
                <div key={item.id} className="flex flex-col items-center justify-center bg-white p-4">
                  <h5 className="break-words text-2xl font-bold text-indigo-800 md:text-3xl">{item.value}</h5>
        ...
        <section id="social-media" ref={registerSection("social-media")} className="scroll-mt-28 p-4">
          <div className="rounded-3xl bg-linear-to-r from-indigo-800 to-violet-600 p-7 text-white">
            ...
            <div className="mt-7 space-y-4">
              {socialMediaItems.length ? (
                socialMediaItems.map((item) => {
                  ...
                  return (
                    <a
                      key={item.id}
                      href={socialUrl}
                      className="block rounded-full border border-indigo-100/50 bg-white/15 p-4"
                ...
                <div className="rounded-full border border-indigo-100/50 bg-white/15 p-4">
                  <div className="mx-auto flex w-fit items-center gap-2">
                    <FontAwesomeIcon icon={faGlobe} className="h-5" />
        ...
        <div className="p-4">
          <div className="rounded-3xl border border-indigo-200 bg-indigo-50 p-7">
            <div className="flex items-center gap-2">
              <img src={safeAlertFill} alt="Alert Icon" className="h-7" />
              <h6 className="text-xl font-bold text-indigo-800">Anti-Fraud Notice</h6>
            </div>
